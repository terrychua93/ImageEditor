...
import getElemDocumentCoords from "../../../ImageEditor/utils/getElemDocumentCoords";
import getPointerOffsetPositionBoundedToObject from "../../../ImageEditor/utils/getPointerOffsetPositionBoundedToObject";
...
let riskAnnotationWrappedRect = null;
...
const previewThenCallAnnotationAdding = (
  ...
  const wrapRiskTextBoundsPreviewByRect = (riskTextAnnotation) => {
    riskAnnotationWrappedRect = getNewAnnotationPreview({
      ...riskTextAnnotation,
      name: TOOLS_IDS.RECT,
      fill: "",
      stroke: "#000000",
    ...
    previewGroup.add(riskAnnotationWrappedRect);
  };
  ...
  const previewAnnotation = (preparedAnnotation) => {
    ...
    if (preparedAnnotation.name === TOOLS_IDS.RISKTEXT) {
      wrapRiskTextBoundsPreviewByRect(preparedAnnotation);
    }
  ...
  const updateAnnotationPreview = (preparedBoundingRect, isShiftKeyPressed) => {
    ...
    if (riskAnnotationWrappedRect) {
      riskAnnotationWrappedRect.setAttrs(transformedAnnotation);
    }
  ...
  const handlePointerUp = () => {
    ...
    riskAnnotationWrappedRect = null;
