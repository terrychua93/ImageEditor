...
import FilerobotImageEditor from '@/components/ImageEditor/components/AssemblyPoint';
import { TABS_IDS, TOOLS_IDS } from '@/components/ImageEditor/utils/constants';
// export { TABS_IDS as TABS, TOOLS_IDS as TOOLS };

/** Services Reference */
import { getFileFileList } from '@/services/idental-api/File';
...
const ImageEditor = (props) => {
  ...
  async function startEditiDentalImage(fileMetadataId) {
    ...
    try {
      await getFileFileList(param).then((result) => {
        const fileData = result[0];
        if (
          fileData &&
          fileData.fileMetadata &&
          fileData.fullFileListByte
        ) {
          const filename = fileData.fileMetadata.fileNameWithoutExtension;
          const fileExt = fileData.fileMetadata.fileExtension.replace('.', '');

          startEdit({ name: filename, type: fileExt, base64Data: fileData.fullFileListByte });
        } else {
    ...
    try {
      await getFileFileList(param).then((result) => {    
        ...
        if (
          ...
          const fileExt = fileData.fileMetadata.fileExtension.replace('.', '');
          const base64 = `data:image/${fileExt};charset=utf-8;base64, ` + fileData.fullFileListByte;
          setDisplayTopbarImageInfo({ base64: base64, fileMetadataId: fileMetadataId, fileExtension: fileData.fileMetadata.fileExtension })
        } else {
          notification.error({ message: 'File data not found' });
        }
      });
