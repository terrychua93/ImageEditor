...
import { HIDE_LOADER, SET_SAVED } from '@/components/ImageEditor/actions';
import {
  ELLIPSE_CROP,
  IMAGE_NODE_ID,
  SUPPORTED_IMAGE_TYPES,
  TOOLS_IDS,
} from '@/components/ImageEditor/utils/constants';
import extractCurrentDesignState from '@/components/ImageEditor/utils/extractCurrentDesignState';
import mapCropBox from '@/components/ImageEditor/utils/mapCropBox';
import getSizeAfterRotation from '@/components/ImageEditor/utils/getSizeAfterRotation';
import imageToBase64 from '@/components/ImageEditor/utils/imageToBase64';
import getFileFullName from '@/components/ImageEditor/utils/getFileFullName';
import operationsToCloudimageUrl from '@/components/ImageEditor/utils/operationsToCloudimageUrl';
...
const useTransformedImgData = () => {
  ...
  const getTransformedImgData = (
      ...
      ...((!imageFileInfo.name || !imageFileInfo.extension) &&
        getFileFullName(
          ...
          forceToPngInEllipticalCrop && crop.ratio === ELLIPSE_CROP
            ? 'png'
            : SUPPORTED_IMAGE_TYPES.includes(defaultSavedImageType?.toLowerCase()) &&
                defaultSavedImageType,
        )),
      ...imageFileInfo,
    };
    ...
    if (size.width) {
      const newScaleX = (isFlippedX ? -1 : 1) * (size.width / preparedCanvas.width());
      preparedCanvas.setAttrs({
        scaleX: newScaleX,
        width: size.width,
    ...
    if (size.height) {
      const newScaleY = (isFlippedY ? -1 : 1) * (size.height / preparedCanvas.height());
      preparedCanvas.setAttrs({
        scaleY: newScaleY,
        height: size.height,
