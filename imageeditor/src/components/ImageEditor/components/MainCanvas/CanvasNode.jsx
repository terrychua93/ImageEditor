...
const ZOOM_DELTA_TO_SCALE_CONVERT_FACTOR = 0.002;
...
const CanvasNode = ({ children }) => {
  ...
  const reSizeOriginalImage = () => {
    if(originalImage.width <= 800){
      originalImage.width = originalImage.width * 2
      originalImage.height = originalImage.height * 2
    }
    if(originalImage.width <= 200){
      originalImage.width = originalImage.width * 3.5
      originalImage.height = originalImage.height * 3.5
    }
  }
  ...
  const dragBoundFunc = (pos) => {
    const x = Math.min(0, Math.max(pos.x, originalImage.width * (1 - zoom.factor)));
    const y = Math.min(0, Math.max(pos.y, originalImage.height * (1 - zoom.factor)));
