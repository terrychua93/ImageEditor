...
import { CHANGE_POINTER_ICON, ENABLE_TEXT_CONTENT_EDIT } from '@/components/ImageEditor/actions';
...
const NodesTransformer = () => {
  ...
  const enableTextContentChangeOnDblClick = () => {
    if (selections.length === 1 && selections[0].name() === TOOLS_IDS.TEXT || selections.length === 1 && selections[0].name() === TOOLS_IDS.RISKTEXT) {
      dispatch({
