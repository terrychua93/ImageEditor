...
const generateCropQuery = (crop, previewDimensions, originalDimensions) => {
  const { x, y, width, height } = mapCropBox(crop, previewDimensions, originalDimensions);

  return `tl_px=${x},${y}&br_px=${x + width},${y + height}${
    crop.ratio === ELLIPSE_CROP ? `&radius=${Math.max(width, height)}&force_format=png` : ''
  }`;
};

const generateResizeQuery = ({ width, height } = {}) => `w=${width}&h=${height}`;
...
const generateWatermarkQuery = (watermarkAnnotation = {}, previewDimensions, crop = {}) => {
  const {
    width,
    height,
    x,
  ...
  if (
    watermarkAnnotation.name === TOOLS_IDS.TEXT ||
    watermarkAnnotation.name === TOOLS_IDS.RISKTEXT
  ) {
    return `${queryParams}&wat_text=${watermark.text.replaceAll('\n', '')}&wat_font=${
      watermark.fontFamily
    }&wat_color=${watermark.fill.replace('#', '')}&wat_fontsize=${watermark.fontSize}max`;
  }
...
const generateFinetuneQuery = (finetunes, finetunesProps = {}) => {
  ...
  finetunes.forEach((finetuneFn) => {
    const finetuneParamInfo = finetuneFn.name && finetuneNameToParamInfo[finetuneFn.name];
...
const operationsToCloudimageUrl = (cloudimage, operations, previewDimensions, originalImage) => {
  const { token, domain, dontPrefixUrl, version, imageSealing, secureProtocol } = cloudimage;
  const {
    imgSrc,
  ...
  if (crop.width && crop.height && (crop.x || crop.x === 0) && (crop.y || crop.y === 0)) {
    operationsQueries.push(generateCropQuery(crop, previewDimensions, originalImage));
  }

  if (resize.width || resize.height) {
    operationsQueries.push(generateResizeQuery({ ...originalImage, ...resize }));
  }
  ...
  if (annotations[WATERMARK_ANNOTATION_ID]) {
    operationsQueries.push(
      generateWatermarkQuery(annotations[WATERMARK_ANNOTATION_ID], previewDimensions, crop),
    );
  }
